...
public class AiResponseValidator {
    ...
    public static AiValidationResult validate(ChatResponse response) {

        if (response == null || response.getResult() == null || response.getResult().getOutput() == null) {
            log.error("[AI_VALIDATOR] 응답 데이터가 비어있거나 파싱할 수 없음");
            return AiValidationResult.fail(AiErrorCode.AI_PARSE_ERROR);
        }
        ...
        if (response.getResult().getMetadata() != null) {
            ...
            if ("length".equalsIgnoreCase(finishReason)) {
                ...
                return AiValidationResult.fail(AiErrorCode.AI_RESPONSE_TRUNCATED);
            }

            if ("content_filter".equalsIgnoreCase(finishReason)) {
                log.warn("[AI_VALIDATOR] 답변 중단: 부적절한 콘텐츠 차단");
                return AiValidationResult.fail(AiErrorCode.AI_CONTENT_POLICY_VIOLATION);
            }
        }
